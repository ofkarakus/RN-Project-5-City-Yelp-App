...
const Countries = (props) => {
  ...
  const renderCountries = ({item}) => <CountryCard country={item.country} navigation={props.navigation}/>

  const [inputValue, setInputValue] = useState('')
  const [filteredCountryList, setFilteredCountryList] = useState([])

  useEffect(() => {setFilteredCountryList(countryList)}, [])
  useEffect(() => {setFilteredCountryList(countryList.filter( x => {
